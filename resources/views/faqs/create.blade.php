...
<div class="modal fade" id="modal-question" tabindex="-1" role="dialog" aria-labelledby="trackModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <form id="form-question" class="form-horizontal">
    <div class="modal-dialog">
        <div class="modal-content">

            <div class="modal-header">
                <h3 class="getlaid" id="trackModalLabel"><i class="ace-icon fa fa-search orange bigger-125"></i> Question</h3>
            </div>
...
<script type="text/javascript">
    ...
    jQuery(function($){
        ...
        $('#form select[name="dialect_id"]').on('change',function(e){
            $.ajax({
                url: "{{url('/get-categories-via-dialect')}}/"+$(this).val(),
                type: 'GET',
                dataType: 'JSON',
                success: function(result){
                    
                    var obj = result;
                    var innerHTML = '<option value="none" selected disabled>--Select category--</option>';
                    for(var i=0; i<obj.length; i++){
                        innerHTML = innerHTML + '<option value="'+obj[i]['id']+'">'+obj[i]['name']+'</option>';
                    }
                    $('#form select[name="category_id"]').html(innerHTML);
                    $('#form select[name="question_id"]').html('');
                }
            })
        })

        $('#form select[name="category_id"]').on('change',function(e){
            $.ajax({
                url: "{{url('/get-questions-via-category')}}/"+$(this).val(),
                type: 'GET',
                dataType: 'JSON',
                success: function(result){
                    
                    var obj = result;
                    var innerHTML = '<option value="none" selected disabled>--Select question--</option><option value="new">--Create new--</option>';
                    for(var i=0; i<obj.length; i++){
                        innerHTML = innerHTML + '<option value="'+obj[i]['id']+'">'+obj[i]['question']+'</option>';
                    }
                    $('#form select[name="question_id"]').html(innerHTML);
                }
            })
        });

        $('#form-question select[name="dialect_id"]').on('change',function(e){
            $.ajax({
                url: "{{url('/get-categories-via-dialect')}}/"+$(this).val(),
                type: 'GET',
                dataType: 'JSON',
                success: function(result){
                    ...
                    $('#form-question select[name="category_id"]').html(innerHTML);
                }
            })
        })
        
        
        $('#form select[name="question_id"]').on('change',function(e){
        ...
        $('#form-question select[name="question_id"]').on('change',function(e){
            var question_id = $(this).val();
            if(question_id=="new"){
                $('#modal-question').modal('show');
            }
        ...
        $('#form-question').validate({
            ...
            submitHandler: function (form) {
                var form_data = new FormData(form);
                $.ajax({
                    url: "{{route('guides.question_store')}}",
                    type: "POST",
                    data: form_data,
                    dataType: 'JSON',
                    processData: false,
                    ...
                    success: function(result){
                        ...
                        if(result.type=="success"){
                            ...
                            $('#form select[name="question_id"] option:selected').removeAttr('selected');
                            $('#form select[name="question_id"]').append('<option selected value="'+result.data.id+'">'+result.data.question+'</option>');
                        }
