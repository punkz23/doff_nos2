  ...
  <div class="page-content">
    ...
    <div class="list">
        ...
        <ul>
            <li>
              <label class="item-radio item-radio-icon-start item-content">
                <input type="radio" name="payment_type" value="CI" required />
          ...
          <li>
            <label class="item-radio item-radio-icon-start item-content">
              <input type="radio" name="payment_type" value="CD" required />
                <i class="icon icon-radio"></i>
                <div class="item-inner">
                  <div class="item-title-row">
              ...
              <li>
                <a class="item-link smart-select smart-select-init" data-open-in="popup"  data-searchbar="true" data-searchbar-placeholder="Search shipper" data-virtual-list="true">
                  <select name="shipper" class="contacts">
                      <option selected value="none" disabled>--PLEASE SELECT SHIPPER--</option>
                      @foreach($contacts as $row)
                        <option data-contact_no="{{$row->contact_no}}" data-user_address="{{$row->user_address}}" value="{{$row->contact_id}}">{{$row->fileas}}</option>
                      @endforeach
                  </select>
              ...
              <li class="item-content item-input">
                <div class="item-inner">
                  ...
                  <div class="item-input-wrap">
                    <input type="text" name="shipper_contact_no" class="shipper_contact_no" readonly>
            ...
            <ul>
              ...
              <li class="item-content item-input">
                <div class="item-inner">
                  <div class="item-title item-label">Contact #</div>
                  <div class="item-input-wrap">
                    <input type="text" name="consignee_contact_no" class="consignee_contact_no" readonly>
                    <span class="input-clear-button"></span>
                  </div>
                </div>
              </li>
      ...
      <ul>
          ...
          <div class="item-inner">
            ...
            <div class="item-input-wrap">
              <select name="shipment_type">
                ...
                <option data-declared_value="1000">BREAKABLE</option>
                <option data-declared_value="1000">PERISHABLE</option>
                <option data-declared_value="500">LETTER</option>
                <option data-declared_value="2000">OTHERS</option>
              </select>
            </div>
          </div>
        </li>
        ...
        <li class="item-content item-input">
          <div class="item-inner">
            <div class="item-title item-label">Declared Value</div>
            <div class="item-input-wrap">
              <input type="number" class="declare-amount" name="declared_value" readonly value="">
              <input type="hidden" name="declared_amount">
            </div>
          </div>
        </li>
    ...
    <script>
        ...
        $('#review').click(function(){
            if($('#form').valid()){
                ...
                if(validated==1){
                    var payment_type_text = $('input[name="payment_type"]:checked').val()=="CI" ? "Prepaid" : "Collect";
                    ...
                    for(var i=0;i<$('.shipments-list > ul li').length;i++){
                        ...
                        description[i]=$('.shipments-list > ul li select.description').eq(i).val();
                        unit[i]=$('.shipments-list > ul li select.units').eq(i).val();
              ...
              if($('input[name="payment_type"]:checked').val()==undefined || $('select[name="shipper"] option:checked').val()=='none' || $('select[name="shipper_address"] option:checked').val()=='none' || $('select[name="consignee"] option:checked').val()=='none' || $('select[name="consignee_address"] option:checked').val()=='none' || $('select[name="destination"] option:selected').val()=='none' || $('select[name="shipment_type"] option:selected').val()=='none'){
                  var toastBottom = app.toast.create({
                    icon: '<i class="icons f7-icons">exclamationmark_triangle</i>',
                    text: 'Please check the required inputs',
                    position: 'center',
                    closeTimeout: 2000,
                  });
                  toastBottom.open();
          ...
          if($('#form').valid()){
                ...
                if(validated==1){
                  ...
                  success: function(data,status,xhr){
                      ...
                      //   text: obj['message']+', Booking Refence # : '+obj['data']['reference_no'],
                      //   position: 'center',
                      //   closeTimeout: 6000,
                      // });
                      // if(obj.type=='success'){
                      ...
                      //   $('#form').trigger('reset');
                      //   setTimeout(function(){
                      //     window.location.href="{{url('/waybills')}}/"+obj['data']['reference_no'];
                      //   },1500);
                      // }
                      // toastBottom.open();
                  },
